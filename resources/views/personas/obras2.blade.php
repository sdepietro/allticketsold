...
<style>
    ...
    <div class="flex flex-col justify-between h-full">
        <div class="flex flex-col items-center justify-center w-full gap-10 py-2 header">
        <div class="grid items-center w-full grid-cols-3 px-5 containerHeader">
          <span>
...
<span class="flex items-center p-2 bg-white buscadorMain">
<form id="frmBuscar" action="{{ route('personas.buscar') }}" method="GET" style="display: flex;justify-content: space-between;width: 100%;">
 ...
 @if (Auth::guard('clientes')->check())
<!---- LOGUEADO --->
 <span class="relative flex flex-col items-center justify-center p-2 px-5 bg-black cursor-pointer w-max justify-self-end btnIngresarDash">
            <input type="checkbox" id="toggle-dropdown-dash" />
            <label class="flex items-center gap-4 cursor-pointer" for="toggle-dropdown-dash">
            ...
            <ul class="items-center gap-4 pt-5 text-white containerMenuDashboard">
...
@else
<!---- SIN LOGUEAR --->
	<span class="flex justify-center p-2 px-5 bg-black rounded-full cursor-pointer w-max justify-self-end btnIngresar">
            <a href="{{ route('personas.login') }}" class="flex gap-4">
              <img src="{{ config('app.url') }}/recursos/public/userBtn.png"></img>
              <p class="text-2xl font-bold text-white uppercase">Ingresa</p>
            </a>
          </span>
<!---- SIN LOGUEAR --->
@endif
...
@if(empty($organiser->twitter))
	...
	@endif
        ...
        <div class="grid items-center grid-cols-2 px-5 py-1 navBarMain">
          <ul class="flex items-center gap-4 text-xl font-semibold text-white listadoCategoriaNav">
             @foreach($categorias3 as $categoria3)
            <li style="display: inline-block;white-space: nowrap;">
               <a href="{{ route('personas.categoria', $categoria3->id) }}">{{ $categoria3->descripcion }}</a>
            </li>
        	@endforeach
          </ul>
            <ul class="flex items-center gap-4 justify-self-end containerSiguenos">
              <span class="text-xl font-bold">SIGUENOS:</span>
             @if(empty($organiser->google_analytics_code))
	@else
       	<li>
        ...
        <ul class="flex justify-end gap-4 text-white containerSubtitleObra">
            <li class="flex items-center gap-2 font-bold text-end">
                <div class="flex items-center gap-2">
                    <img src="./public/obrasPage/iconocategoria.png" alt="">
            ...
            <li class="flex items-center gap-2 font-bold text-end" style="display:none">
                <div class="flex items-center gap-2">
                    <img src="./public/obrasPage/icono2.png" alt="">
            ...
            <li class="flex items-center gap-2 font-bold text-end">
                <div class="flex items-center gap-2">
                    <img src="./public/obrasPage/iconoreloj.png" alt="">
            ...
            <div class="containerTicketsPrecio">
                    <div class="flex flex-col gap-4">
                      <span class="textoResaltado tituloSeccionObra">Tickets en Venta:</span>
                        <div style="display: flex;justify-content: center;"><img class="imgRes" src="./public/obrasPage/iconoticket.png" alt=""></div>
                    </div>
...
<script>
    var grandTotal = @json($tickets);
    console.log(grandTotal);
	function sinEntradas()
	{
		alert("No hay entradas disponibles.")
                    ...
                    <div class="flex flex-col items-center justify-between gap-4 pt-3" style="justify-content: flex-start !important;">
                        ...
                        <img src="./public/obrasPage/logo-payway.svg" alt="bancolanacion" style="filter: invert(50%);max-width: 60%;margin-bottom: 20px;">
@if (Auth::guard('clientes')->check())
...
@endforeach
{!! Form::text("order_first_name", Auth::guard('clientes')->user()->nombres, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}

{!! Form::text("order_last_name", Auth::guard('clientes')->user()->id, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}
{!! Form::text("order_email", Auth::guard('clientes')->user()->email, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}

{!! Form::text("ticket_holder_first_name", Auth::guard('clientes')->user()->nombres, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}
{!! Form::text("ticket_holder_last_name", Auth::guard('clientes')->user()->id, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}
{!! Form::text("ticket_holder_email", Auth::guard('clientes')->user()->email, ['required' => 'required', 'class' => 'form-control', 'style' => 'display: none;']) !!}
                        <button style="width: 100%;" class="w-2/4 px-8 py-5 text-2xl text-white btnComprarTicket">COMPRAR TICKETS</button>
{!! Form::close() !!}
@else
<button class="w-2/4 px-8 py-5 text-2xl text-white btnComprarTicket" onclick="alert('Debe iniciar sesión para comprar.')">COMPRAR TICKETS</button>
@endif
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col gap-4 p-5 rounded-lg bgCardObra">
            ...
            <div class="flex gap-4">
                <div class="containerSinopsis">
                    <div>
                        <p class="text-white text-size-normal">{{ $event->description }}</p>
                    </div>
        ...
        <div class="flex flex-col gap-4 p-5 rounded-lg bgCardObra">
            <div class="flex justify-between containerLugarObras">
                ...
                <div class="flex items-center gap-4" style="justify-content: flex-end;">
                    ...
                    <img src="./public/obrasPage/ubicacionLugar.png" alt="">
...
<script>
        document.getElementById('whatsappShare').addEventListener('click', function() {
            var text = encodeURIComponent('Mira esta Obra '); // Cambia el texto aquí
            var url = '{{ url()->current() }}';
