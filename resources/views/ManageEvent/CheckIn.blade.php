...
<head>
    <title>
        @lang("Allticket.check_in", ["event"=>$event->title])
    </title>
            ...
            <div class="col-md-12">
                <div class="attendee_list">
                    ...
                    <ul v-if="searchResultsCount > 0" class="list-group" id="attendee_list" v-cloak>
                        ...
                        style="display: flex;align-items: center;justify-content: space-between;height: 75px;">
						<div style="display: flex;align-items: center;">
                            <div style="margin-left: 10px;">@lang("Attendee.name"): <b>@{{ attendee.first_name }} </b> </div>
				<div style="margin-left: 10px;">DNI: <b>@{{ attendee.last_name }} </b> </div>
							
							<div style="margin-left: 10px;"><span v-if="!attendee.is_payment_received" class="label label-danger">@lang("Order.awaiting_payment")</span></div>
                    
                            <div style="margin-left: 10px;">@lang("Order.reference"): <b>@{{ attendee.order_reference + '-' + attendee.reference_index }}</b></div>
                   
                            <div style="margin-left: 10px;">@lang("Order.ticket"): <b>@{{ attendee.ticket }}</b></div>
						</div>
						<div>
						<b>Ingreso a sala:</b><span style="font-size: medium;font-weight: 600;color: @{{ attendee.has_arrived ? 'green' : 'red' }};">
    @{{ attendee.has_arrived ? 'Sí' : 'No' }}
</span>
						</div>
