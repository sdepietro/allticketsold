...
@if ($errors->any())
    ...
    @endif
    <form action="{{ route('preguntas.update', [$organiser_id, $pregunta->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label class="control-label" for="pregunta">Pregunta</label>
            <input class="form-control" type="text" name="pregunta" value="{{ $pregunta->pregunta }}" required>
        </div>
        <div class="form-group">
            <label class="control-label" for="respuesta">Respuesta</label>
            <textarea class="form-control" name="respuesta" required>{{ $pregunta->respuesta }}</textarea>
        </div>
        <div style="display:none">
            <label class="control-label" for="activado">Activado</label>
            <input type="checkbox" name="activado" value="0">
        </div>
