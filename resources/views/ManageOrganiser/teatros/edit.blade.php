...
@section('content')
    ...
    <form action="{{ route('teatros.update', ['organiser_id' => $organiser_id, 'id' => $teatro->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        
        <div class="form-group">
            <label class="control-label" for="nombre">Nombre</label>
            <input class="form-control" type="text" name="nombre" id="nombre" class="form-control" value="{{ old('nombre', $teatro->nombre) }}" required>
            @error('nombre')
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="direccion" id="direccion" class="form-control" value="{{ old('direccion', $teatro->direccion) }}" required>
            @error('direccion')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="coordenadas" id="coordenadas" class="form-control" value="{{ old('coordenadas', $teatro->coordenadas) }}" required>
            @error('coordenadas')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            <label class="control-label" for="imagen">Imagen (Dejar en blanco si no se quiere cambiar)</label>
            <input class="form-control" type="file" name="imagen" id="imagen" class="form-control">
            @error('imagen')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
            @if($teatro->imagen)
                <img src="{{ asset('storage/' . $teatro->imagen) }}" alt="Imagen del Teatro" style="width: 150px; margin-top: 10px;">
            @endif
        </div>
		
		<div class="form-group">
            <label class="control-label" for="map">Ubicación en el Mapa</label>
            <div id="map" style="height: 400px; width: 100%;"></div>
        </div>

        <button type="submit" class="btn btn-primary">Actualizar Teatro</button>
