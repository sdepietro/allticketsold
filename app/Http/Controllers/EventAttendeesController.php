...
class EventAttendeesController extends MyBaseController
{
    ...
    public function postInviteAttendee(Request $request, $event_id)
    {
		...
		$ticket2 = Ticket::scope()->find($ticket_id);
		
		$restantes = $ticket2->quantity_available - $ticket2->quantity_sold;
		if ($restantes <= 0) {
			session()->flash('message', 'No hay ticket disponibles');
			return response()->json([
                'status'      => 'success',
    ...
    public function postImportAttendee(Request $request, $event_id)
    {
		...
		$ticket_id2 = $request->get('ticket_id');
		
		$ticket2 = Ticket::scope()->find($ticket_id2);
		...
		//$mensaje = $request->file('attendees_list');
		//session()->flash('message', $mensaje->getClientOriginalName());
    ...
    public function postMessageAttendees(Request $request, $event_id)
    {
       ...
       // $validator = Validator::make($request->all(), $rules);
		...
		if ($recipients != 'all') {
			$recipients = array_filter(explode(',', $recipients), function($email) {
    ...
    public function postResendTicketToAttendee(Request $request, $attendee_id)
    {
        ...
        $this->dispatch(new SendOrderAttendeeTicketJob($attendee, $ticket_url));
