    ...
    public function showEvents(Request $request, $organiser_id)
    {
        ...
        /*$events = $searchQuery
            ? Event::scope()->with(['organiser', 'currency'])->where('title', 'like', '%' . $searchQuery . '%')->orderBy($sort_by,
                'desc')->where(function ($query) { $query->where('location_address_line_1', '<>', '1')->orWhereNull('location_address_line_1');})->where('location_address_line_1', '!=', '1111')->where('organiser_id', '=', $organiser_id)->paginate(12)
